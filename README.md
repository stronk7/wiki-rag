...
## Configuration
 ...
 1. **Set up Configuration File**
   ...
   The application supports both YAML configuration (recommended) and legacy `.env` files.
   ...
   **Option A: Using config.yaml (Recommended)**
   ...
   - Copy the configuration template:
      ...
      ```
   - Edit `config.yaml` to suit your needs (mediawiki site and namespaces, models, etc.)

   **Option B: Using .env (Legacy, will be deprecated)**

   - If you have an existing `.env` file, you can convert it to `config.yaml`:
      ```bash
      wr-config-update
      ```
   - This will create `config.yaml` from your `.env` file while maintaining backward compatibility.
...
4. **Run the Application**:
    The application comes with five different executables:
    - `wr-config-update`: Converts `.env` file to `config.yaml` format (for migration from legacy configuration).
...
2. Run the container:
    - **Note 1:** Don't forget to have the `config.yaml` file (or `.env` for backward compatibility) in the same directory as the command below.
    - **Note 2:** The `data` directory will be created in the current directory, and it will store all the data generated by the `wr-load` command. If the `config.collection.dump_path` is set, you will have to change the volume mapping accordingly.
    ```bash
    ...
    - **Note 4:** The 2 lines related to Milvus are required to connect to the Milvus server **if also running in Docker**. If it's running elsewhere, you can replace the `MILVUS_URL` with the appropriate URL or configure it in the `config.yaml` file instead.
