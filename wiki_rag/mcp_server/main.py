...
def main():
    ...
    if config.get_bool("langsmith.prompts", False):
        if not config.get("langsmith.endpoint"):
            logger.error("LANGSMITH_ENDPOINT (required if prompts is enabled) not found in configuration. Exiting.")
            sys.exit(1)
        if not config.get("langsmith.api_key"):
    ...
    if config.get_bool("langfuse.prompts", False):
        if not config.get("langfuse.host"):
            logger.error("LANGFUSE_HOST (required if prompts is enabled) not found in configuration. Exiting.")
            sys.exit(1)
        if not config.get("langfuse.public_key"):
            logger.error("LANGFUSE_PUBLIC_KEY (required if prompts is enabled) not found in configuration. Exiting.")
            sys.exit(1)
        if not config.get("langfuse.secret_key"):
            logger.error("LANGFUSE_SECRET_KEY (required if prompts is enabled) not found in configuration. Exiting.")
            sys.exit(1)
    ...
    mcp_api_base = config.get("mcp.api_base", "0.0.0.0:8081")
